                                     ...
                                     const bool required)
{
    ...
    if (node.empty())
    {
        if (required)
        {
            LOG(ERROR) << name << " required parameter does not exist, aborting..." << std::endl;
            exit(-1);
        }
        else
        {
            LOG(ERROR) << name << " optional parameter does not exist..." << std::endl;
            found = false;
            return 0.0f;
        }
    }
    ...
    {
        LOG(ERROR) << name << " parameter must be a real number, aborting..." << std::endl;
        exit(-1);
    }
    else
    {
...
int Settings::readParameter<int>(cv::FileStorage& fSettings, const std::string& name, bool& found, const bool required)
{
    ...
    {
        LOG(ERROR) << name << " parameter must be an integer number, aborting..." << std::endl;
        exit(-1);
    }
    else
    {
                                       ...
                                       const bool required)
{
    ...
    {
        LOG(ERROR) << name << " parameter must be a string, aborting..." << std::endl;
        exit(-1);
    }
    else
    {
  ...
  : bNeedToUndistort_(false), bNeedToRectify_(false), bNeedToResize1_(false), bNeedToResize2_(false)
{
    ...
    {
        LOG(INFO) << "Loading settings from " << configFile;
    }

    // Read first camera
    readCamera1(fSettings);
    LOG(INFO) << "\t-Loaded camera 1";

    // Read second camera if stereo (not rectified)
    if (sensor_ == System::STEREO || sensor_ == System::IMU_STEREO)
    {
        readCamera2(fSettings);
        LOG(INFO) << "\t-Loaded camera 2";
    }
    ...
    {
        readIMU(fSettings);
        LOG(INFO) << "\t-Loaded IMU calibration";
    }

    if (sensor_ == System::RGBD || sensor_ == System::IMU_RGBD)
    {
        readRGBD(fSettings);
        LOG(INFO) << "\t-Loaded RGB-D calibration";
    }

    readORB(fSettings);
    LOG(INFO) << "\t-Loaded ORB settings";
    readViewer(fSettings);
    LOG(INFO) << "\t-Loaded viewer settings";
    readLoadAndSave(fSettings);
    LOG(INFO) << "\t-Loaded Atlas settings";
    readOtherParameters(fSettings);
    LOG(INFO) << "\t-Loaded misc parameters";

    if (bNeedToRectify_)
    {
        precomputeRectificationMaps();
        LOG(INFO) << "\t-Computed rectification maps";
    }
...
void Settings::readCamera1(cv::FileStorage& fSettings)
{
    ...
    if (cameraModel == "PinHole")
    {
        ...
        if ((sensor_ == System::MONOCULAR || sensor_ == System::IMU_MONOCULAR || sensor_ == System::RGBD ||
