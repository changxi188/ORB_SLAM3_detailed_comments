...
void LoopClosing::Run()
{
    ...
    LOG(INFO) << "Run --- Start LoopClosing";
    ...
    while (1)
    {
        ...
        // LOG(INFO) << "Run --- loopclosing thread is running";
        ...
        {
            // LOG(WARNING) << "Run --- no new keyframe in buffer.";
            // 查看是否有外部线程请求复位当前线程
            ResetIfRequested();
        ...
        LOG(INFO) << "Run --- LoopClosing thread get new key frames";
        ...
        double timePRTotal =
            std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndPR - time_StartPR).count();
        ...
        if (mbMergeDetected)
        {
            ...
            {
                LOG(INFO) << "Run --- IMU is not initilized, merge is aborted";
            ...
            {
                LOG(INFO) << "Merge check transformation with IMU";
                ...
                if (mSold_new.scale() < 0.90 || mSold_new.scale() > 1.1)
                {
                    ...
                    mnMergeNumNotFound = 0;
                    mbMergeDetected    = false;
                    LOG(INFO) << "scale bad estimated. Abort merging";
                    continue;
                }
            ...
            double timeMergeTotal =
                std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndMerge - time_StartMerge)
                    .count();
        ...
        if (mbLoopDetected)
        {
            ...
            if (mpCurrentKF->GetMap()->IsInertial())
            {
                ...
                // mg2oLoopScw是通过回环检测的Sim3计算出的回环矫正后的当前关键帧的初始位姿,
                // Twc是当前关键帧回环矫正前的位姿. g2oSww_new 可以理解为correction
                g2o::Sim3 g2oSww_new = g2oTwc * mg2oLoopScw;

                // 拿到 roll ,pitch ,yaw
                Eigen::Vector3d phi = LogSO3(g2oSww_new.rotation().toRotationMatrix());
                cout << "phi = " << phi.transpose() << endl;
                // 这里算是通过imu重力方向验证回环结果, 如果pitch或roll角度偏差稍微有一点大,则回环失败.
                // 对yaw容忍比较大(20度)
            ...
            if (bGoodLoop)
            {
                ...
                double timeLoopTotal =
                    std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndLoop - time_StartLoop)
                        .count();
...
{
    LOG(INFO) << "NewDetectCommonRegions --- begin detect common regions.";
    ...
    {
        LOG(INFO) << "NewDetectCommonRegions --- map is not over iniertial ba2, keyframe id : " << mpCurrentKF->mnId;
    ...
    {
        LOG(INFO) << "NewDetectCommonRegions --- Stereo KF inserted without check: " << mpCurrentKF->mnId;
        mpKeyFrameDB->add(mpCurrentKF);
        mpCurrentKF->SetErase();
        return false;
    }
    ...
    {
        LOG(INFO) << "NewDetectCommonRegions --- Stereo KF inserted without check, map is small: " << mpCurrentKF->mnId;
        mpKeyFrameDB->add(mpCurrentKF);
        mpCurrentKF->SetErase();
        return false;
    }

    LOG(INFO) << "NewDetectCommonRegions --- Checking KF id : " << mpCurrentKF->mnId << endl;

    // Check the last candidates with geometric validation
    //  Loop candidates
    //  Step 3
    //  基于前一帧的历史信息判断是否进行时序几何校验,注意这里是基于共视几何校验失败才会运行的代码,阅读代码的时候可以先看后面
    ...
    double timeEstSim3 =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndEstSim3_1 - time_StartEstSim3_1)
            .count();
#endif
    ...
    if (!bMergeDetectedInKF || !bLoopDetectedInKF)
    {
        ...
        LOG(INFO) << "NewDetectCommonRegions --- detect n best candidates, vpLoopBowCand size : "
                  << vpLoopBowCand.size() << ", vpMergeBowCand size : " << vpMergeBowCand.size();
#ifdef REGISTER_TIMES
        std::chrono::steady_clock::time_point time_EndQuery = std::chrono::steady_clock::now();

        double timeDataQuery =
            std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndQuery - time_StartQuery)
                .count();
    ...
    if (!bLoopDetectedInKF && !vpLoopBowCand.empty())
    {
        ...
        mbLoopDetected = DetectCommonRegionsFromBoW(vpLoopBowCand, mpLoopMatchedKF, mpLoopLastCurrentKF, mg2oLoopSlw,
    ...
    if (!bMergeDetectedInKF && !vpMergeBowCand.empty())
    {
        ...
        mbMergeDetected =
            DetectCommonRegionsFromBoW(vpMergeBowCand, mpMergeMatchedKF, mpMergeLastCurrentKF, mg2oMergeSlw,
    ...
    timeEstSim3 +=
        std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndEstSim3_2 - time_StartEstSim3_2)
            .count();
    vdEstSim3_ms.push_back(timeEstSim3);
#endif
                                                 ...
                                                 std::vector<MapPoint*>& vpMatchedMPs)
{
    ...
    {
        LOG(WARNING) << "DetectAndReffineSim3FromLastKF --- Sim3 reffine: There are " << nNumProjMatches
                     << " initial matches, less than require : " << nProjMatches;
        return false;
    }

    LOG(INFO) << "DetectAndReffineSim3FromLastKF --- Sim3 reffine: There are " << nNumProjMatches
              << " initial matches ";
    ...
    // 3.2 优化gScm，mp固定
    int numOptMatches =
        Optimizer::OptimizeSim3(mpCurrentKF, pMatchedKF, vpMatchedMPs, gScm, 10, bFixedScale, mHessian7x7, true);

    // Verbose::PrintMess("Sim3 reffine: There are " + to_string(numOptMatches) + " matches after of the
    // optimization ", Verbose::VERBOSITY_DEBUG);
    //  若匹配的数量大于一定的数目
    if (numOptMatches <= nProjOptMatches)
    {
        LOG(WARNING) << "DetectAndReffineSim3FromLastKF --- Sim3 reffine: There are " << numOptMatches
                     << " matches after of the optimization, less than require " << nProjOptMatches;
        return false;
    }

    LOG(INFO) << "DetectAndReffineSim3FromLastKF --- Sim3 reffine: There are " << numOptMatches
              << " matches after of the optimization ";
    ...
    // 再次通过优化后的Sim3搜索匹配点
    nNumProjMatches =
        FindMatchesByProjection(pCurrentKF, pMatchedKF, gScw_estimation, spAlreadyMatchedMPs, vpMPs, vpMatchedMPs);
    // 若果大于期望数目,接受这个结果
    if (nNumProjMatches >= nProjMatchesRep)
    {
        LOG(INFO) << "DetectAndReffineSim3FromLastKF --- Sim3 reffine: There are " << nNumProjMatches << " matches";
        gScw = gScw_estimation;
        // 验证成功
        return true;
    }
    LOG(INFO) << "DetectAndReffineSim3FromLastKF --- Sim3 reffine: There are " << nNumProjMatches
              << " matches, less than require : " << nProjMatchesRep;
    // 验证失败
    return false;
}
                                             ...
                                             std::vector<MapPoint*>& vpMPs, std::vector<MapPoint*>& vpMatchedMPs)
{
    ...
    LOG(INFO) << "DetectCommonRegionsFromBoW --- BoW candidates: There are " << vpBowCand.size()
              << " possible candidates ";
    // Verbose::PrintMess("BoW candidates: There are " + to_string(vpBowCand.size()) + " possible candidates ",
    // Verbose::VERBOSITY_DEBUG);
    //  2. 对每个候选关键帧都进行详细的分析
    for (KeyFrame* pKFi : vpBowCand)
    {
        ...
        LOG(INFO) << "DetectCommonRegionsFromBoW --- KF candidate id : " << pKFi->mnId;
        ...
        if (vpCovKFi.empty())
        {
            LOG(INFO) << "DetectCommonRegionsFromBoW --- Covisible list empty";
        ...
        // 检测窗口内是否有当前关键帧的共视关键帧，如果有则提前结束此次循环
        ...
        if (bAbortByNearKF)
        {
            LOG(INFO) << "DetectCommonRegionsFromBoW --- Check BoW aborted because is close to the matched one ";
            continue;
        }

        LOG(INFO) << "DetectCommonRegionsFromBoW --- Check BoW continue because is far to the matched one ";
        ...
        std::vector<MapPoint*> vpMatchedPoints =
            std::vector<MapPoint*>(mpCurrentKF->GetMapPointMatches().size(), static_cast<MapPoint*>(NULL));
        // 记录上面的地图点分别对应窗口中的关键帧(数量的上限是当前关键帧地图点的数量)
        std::vector<KeyFrame*> vpKeyFrameMatchedMP =
            std::vector<KeyFrame*>(mpCurrentKF->GetMapPointMatches().size(), static_cast<KeyFrame*>(NULL));
        ...
        // 2.
        // 有可能作者认为在DetectNBestCandidates已经找到共视关键帧中分数最多的了，所以这里不做判断直接使用原始的候选关键帧
        ...
        {
            LOG(WARNING) << "DetectCommonRegionsFromBoW --- Number of matched search by BoW " << numBoWMatches
                         << ", less than minimum require : " << nBoWMatches;
            index++;
            continue;
        }

        LOG(INFO) << "DetectCommonRegionsFromBoW --- Number of matched search by BoW " << numBoWMatches;
        ...
        Sim3Solver solver =
            Sim3Solver(mpCurrentKF, pMostBoWMatchesKF, vpMatchedPoints, bFixedScale, vpKeyFrameMatchedMP);
        ...
        while (!bConverge && !bNoMore)
        {
            ...
            LOG(INFO) << "DetectCommonRegionsFromBoW ---  Sim3Solver achieve " << nInliers
                      << " geometrical inliers, threshold is " << nBoWInliers << " BoW matches";
            // Verbose::PrintMess("BoW guess: Solver achieve " + to_string(nInliers) + " geometrical inliers among "
        ...
        if (!bConverge)
        {
            LOG(WARNING) << "DetectCommonRegionsFromBoW --- SolverSim3 not converged.";
            index++;
            continue;
        }

        LOG(INFO) << "DetectCommonRegionsFromBoW --- Check BoW: SolverSim3 converged";
        ...
        LOG(INFO) << "DetectCommonRegionsFromBoW --- New local window, There are " << vpCovKFi.size() << " near KFs";
        ...
        LOG(INFO) << "DetectCommonRegionsFromBoW --- There are " << vpMapPoints.size() << " mappoints in local window";

        // 拿到solver 估计的 Scm初始值, 为后续的非线性优化做准备, 在这里 c 表示当前关键帧, m 表示回环/融合候选帧
        g2o::Sim3 gScm(solver.GetEstimatedRotation().cast<double>(), solver.GetEstimatedTranslation().cast<double>(),
        ...
        int numProjMatches =
            matcher.SearchByProjection(mpCurrentKF, mScw, vpMapPoints, vpKeyFrames, vpMatchedMP, vpMatchedKF, 8, 1.5);

        // 如果拿到了足够多的匹配点, nProjMatches = 50
        if (numProjMatches < nProjMatches)
        {
            LOG(INFO) << "DetectCommonRegionsFromBoW ---  use coarse Sim3 search by projection, get match number : "
                      << numProjMatches << ", less than require " << nProjMatches;
            index++;
            continue;
        }
        ...
        // pKFi是候选关键帧
        int numOptMatches =
            Optimizer::OptimizeSim3(mpCurrentKF, pKFi, vpMatchedMP, gScm, 10, mbFixScale, mHessian7x7, true);

        // 3.3.3 如果内点足够多,用更小的半径搜索匹配点,并且再次进行优化(p.s.这里与论文不符,并没有再次优化)
        if (numOptMatches < nSim3Inliers)
        {
            LOG(WARNING) << "DetectCommonRegionsFromBoW --- use optimizer get sim3's inliers num : " << numOptMatches
                         << ", less than require " << nSim3Inliers;
            index++;
            continue;
        }
        LOG(INFO) << "DetectCommonRegionsFromBoW --- use optimizer get sim3's inliers num : " << numOptMatches;

        // 前面已经声明了这些变量了,无需再次声明
        g2o::Sim3     new_gSmw(pMostBoWMatchesKF->GetRotation().cast<double>(),
                               pMostBoWMatchesKF->GetTranslation().cast<double>(), 1.0);
        g2o::Sim3     new_gScw = gScm * new_gSmw;  // Similarity matrix of current from the world position
        Sophus::Sim3f new_mScw = Converter::toSophus(new_gScw);

        vector<MapPoint*> new_vpMatchedMP;
        new_vpMatchedMP.resize(mpCurrentKF->GetMapPointMatches().size(), static_cast<MapPoint*>(NULL));
        // 3.3.4 重新利用之前计算的mScw信息, 通过更小的半径和更严格的距离的投影寻找匹配点
        // 5 : 半径的增益系数(对比之前下降了)---> 更小的半径, 1.0 , hamming distance 的阀值增益系数--->
        // 允许更小的距离
        int numProjOptMatches = matcher.SearchByProjection(mpCurrentKF, new_mScw, vpMapPoints, new_vpMatchedMP, 5, 1.0);

        // 当新的投影得到的内点数量大于nProjOptMatches=80时
        if (numProjOptMatches < nProjOptMatches)
        {
            LOG(WARNING) << "DetectCommonRegionsFromBoW --- use optimized sim3 Project matches : " << numProjOptMatches
                         << ", less than require " << nProjOptMatches;
            index++;
            continue;
        }
        LOG(INFO) << "DetectCommonRegionsFromBoW --- use optimized sim3 Project matches : " << numProjOptMatches;

        /// 以下为调试信息
        int max_x = -1, min_x = 1000000;
        int max_y = -1, min_y = 1000000;
        for (MapPoint* pMPi : new_vpMatchedMP)
        {
            if (!pMPi || pMPi->isBad())
            {
                continue;
        ...
        while (nNumKFs < 3 && j < vpCurrentCovKFs.size())
        {
            ...
            g2o::Sim3         gSjw             = gSjc * new_gScw;
            int               numProjMatches_j = 0;
            vector<MapPoint*> vpMatchedMPs_j;
            // 4.2 几何校验函数, 这个函数里面其实是个searchByProjection :
            // 通过之前计算的位姿转换地图点并通过投影搜索匹配点, 若大于一定数目的任务成功验证一次
            bool bValid = DetectCommonRegionsFromLastKF(pKFj, pMostBoWMatchesKF, gSjw, numProjMatches_j, vpMapPoints,
                                                        vpMatchedMPs_j);
    ...
    if (nBestMatchesReproj > 0)
    {
        ...
        LOG(INFO) << "DetectCommonRegionsFromBoW --- find common regions, number coincidence : " << nNumCoincidences;
    ...
    LOG(INFO) << "DetectCommonRegionsFromBoW --- not find common regions";
    // 如果少于3个当前关键帧的共视关键帧验证了这个候选帧,那么返回失败,注意,这里的失败并不代表最终的验证失败,后续会开启时序校验
    return false;
}

/**
 * @brief 用来验证候选帧的函数, 这个函数的名字取的不好,
 * 函数的本意是想利用当前帧的共视关键帧来验证候选帧,不如改叫做：DetectCommonRegionsFromCoVKF
 *
 * @param[in] pCurrentKF 当前关键帧
 * @param[in] pMatchedKF 候选帧
...
void LoopClosing::CorrectLoop()
{
    ...
    // 对地图点操作
    {
        ...
        // 3.1：通过mg2oLoopScw（认为是准的）来进行位姿传播，得到当前关键帧的共视关键帧的世界坐标系下Sim3
        // 位姿（还没有修正） 遍历"当前关键帧组""
        ...
        // Correct all MapPoints obsrved by current keyframe and neighbors, so that they align with the other side of
        // the loop 3.2：得到矫正的当前关键帧的共视关键帧位姿后，修正这些关键帧的地图点
    ...
    // Fuse duplications.
    // Step 5. 将闭环相连关键帧组mvpLoopMapPoints
    // 投影到当前关键帧组中，进行匹配，融合，新增或替换当前关键帧组中KF的地图点 因为 闭环相连关键帧组mvpLoopMapPoints
    // 在地图中时间比较久经历了多次优化，认为是准确的 而当前关键帧组中的关键帧的地图点是最近新计算的，可能有累积误差
    ...
    double timeFusion =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndFusion - time_StartFusion)
            .count();
    ...
    {
        Optimizer::OptimizeEssentialGraph4DoF(pLoopMap, mpLoopMatchedKF, mpCurrentKF, NonCorrectedSim3, CorrectedSim3,
                                              LoopConnections);
    }
    else
    {
    ...
    double timeOptEss =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndOpt - time_EndFusion).count();
    ...
    // Launch a new thread to perform Global Bundle Adjustment (Only if few keyframes, if not it would take too much
    // time) 闭环地图没有imu初始化或者 仅有一个地图且内部关键帧<200时才执行全局BA，否则太慢
...
void LoopClosing::MergeLocal()
{
    ...
    // Relationship to rebuild the essential graph, it is used two times, first in the local window and later in the
    // rest of the map
    //  建立本质图必须的量
    KeyFrame* pNewChild;
    KeyFrame* pNewParent;
    ...
    // Later, the elements of the current map will be transform to the new active map reference, in order to keep real
    // time tracking 当前关键帧的共视关键帧和他们观测到的地图点会先被融合, 融合后的图会变成新的当前激活地图. 之后,
    // 所有当前地图的其他部分会被转换到当前地图中, 这样是为了保证tracking的实时性.
    ...
    // to_string(spLocalWindowKFs.size()) << "; #MPs = " << to_string(spLocalWindowMPs.size()) << std::endl;
    ...
    // to_string(spMergeConnectedKFs.size()) << "; #MPs = " << to_string(spMapPointMerge.size()) << std::endl;
    ...
    for (KeyFrame* pKFi : spLocalWindowKFs)
    {
        ...
        if (pCurrentMap->isImuInitialized())
        {
            Eigen::Quaternionf Rcor =
                (g2oCorrectedSiw.rotation().inverse() * vNonCorrectedSim3[pKFi].rotation()).cast<float>();
    ...
    {
        std::cout << "[Merge]: " << std::to_string(numPointsWithCorrection) << " points removed from Ma due to its
    reference KF is not in welding area" << std::endl; std::cout << "[Merge]: Ma has " <<
    std::to_string(spLocalWindowMPs.size()) << " points" << std::endl;
    }*/
    // Step 3
    // 两个地图以新（当前帧所在地图）换旧（融合帧所在地图），包括关键帧及地图点关联地图的以新换旧、地图集的以新换旧
    {
        // 当前地图会被更新，老的地图中的重复地图点会被剔除
        unique_lock<mutex> currentLock(pCurrentMap->mMutexMapUpdate);  // We update the current map with the Merge
                                                                       // information
        unique_lock<mutex> mergeLock(pMergeMap->mMutexMapUpdate);  // We remove the Kfs and MPs in the merged area from
                                                                   // the old map
    ...
    pNewParent = mpCurrentKF;  // Old child, now it will be the parent of its own parent(we need eliminate this KF from
    ...
    double timeMergeMaps =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_StartWeldingBA - time_StartMerge)
            .count();
    ...
    double timeWeldingBA =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndWeldingBA - time_StartWeldingBA)
            .count();
    ...
    else
    {
...

        {
            ...
            unique_lock<mutex> currentLock(pCurrentMap->mMutexMapUpdate);  // We update the current map with the Merge
                                                                           // information
            unique_lock<mutex> mergeLock(pMergeMap->mMutexMapUpdate);  // We remove the Kfs and MPs in the merged area
                                                                       // from the old map
    ...
    double timeOptEss =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndOptEss - time_EndWeldingBA)
            .count();
...
void LoopClosing::MergeLocal2()
{
    ...
    vector<MapPoint*> vpCheckFuseMapPoint;  // MapPoint vector from current map to allow to fuse duplicated points with
                                            // the old map (merge)
 ...
 * @param vpMapPoints
 * 待融合地图的融合帧及其5个共视关键帧对应的mp（1000个以内）（注意此时所有kf与mp全部移至当前地图，这里的待融合地图的说法只为区分，因为还没有融合）
 */
void LoopClosing::SearchAndFuse(const KeyFrameAndPose& CorrectedPosesMap, vector<MapPoint*>& vpMapPoints)
{
         ...
         mit++)
    {
        ...
        for (int i = 0; i < nLP; i++)
        {
            ...
            // 也就是之前某次matcher.Fuse
            // 把老点放入到新的关键帧中，下次遍历时，如果老点已经在被代替点的对应的某一个关键帧内
...
void LoopClosing::RunGlobalBundleAdjustment(Map* pActiveMap, unsigned long nLoopKF)
{
    ...
    double timeGBA =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndGBA - time_StartFGBA).count();
        ...
        if (!mbStopGBA)
        {
            ...
            while (!lpKFtoCheck.empty())
            {
                ...
                for (set<KeyFrame*>::const_iterator sit = sChilds.begin(); sit != sChilds.end(); sit++)
                {
                    ...
                    {
                        // cout << "++++New child with flag " << pChild->mnBAGlobalForKF << "; LoopKF: " << nLoopKF <<
                        // endl; cout << " child id: " << pChild->mnId << endl;
                ...
                if(dist > 1)
                {
                    cout << "--To much distance correction: It has " << pKF->GetConnectedKeyFrames().size() << "
                connected KFs" << endl; cout << "--It has " << pKF->GetCovisiblesByWeight(80).size() << " connected KF
                with 80 common matches or more" << endl; cout << "--It has " << pKF->GetCovisiblesByWeight(50).size() <<
                " connected KF with 50 common matches or more" << endl; cout << "--It has " <<
                pKF->GetCovisiblesByWeight(20).size() << " connected KF with 20 common matches or more" << endl;
                    ...
                    for(int i=0; i<vpMapPointsKF.size(); ++i)
                    {
                        ...
                        cv::putText(imLeft, strNumOBs, pKF->mvKeys[i].pt, CV_FONT_HERSHEY_DUPLEX, 1, cv::Scalar(255, 0,
                0));
                    }
                    cout << "--It has " << num_MPs << " MPs matched in the map" << endl;

                    string namefile = "./test_GBA/GBA_" + to_string(nLoopKF) + "_KF" + to_string(pKF->mnId) +"_D" +
                to_string(dist) +".png"; cv::imwrite(namefile, imLeft);
                }*/
            ...
            double timeUpdateMap =
                std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndUpdateMap - time_EndGBA)
                    .count();
            vdUpdateMap_ms.push_back(timeUpdateMap);

            double timeFGBA = std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndUpdateMap -
                                                                                                    time_StartFGBA)
                                  .count();
