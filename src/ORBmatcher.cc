            ...
            if (!vIndices.empty())
            {
                ...
                for (vector<size_t>::const_iterator vit = vIndices.begin(), vend = vIndices.end(); vit != vend; vit++)
                {
                    ...
                    if (F.Nleft == -1 && F.mvuRight[idx] > 0)
                    {
                        ...
                        // 相机运动时对其产生的影响也就越大, 因此需要扩大其搜索空间. 当给定缩放倍率为1.2的时候,
                        // mvScaleFactors 中的数据是: 1 1.2 1.2^2 1.2^3 ...
                ...
                if (bestDist <= TH_HIGH)
                {
                    ...
                    // 条件2：bestDist>mfNNratio*bestDist2 表示最佳和次佳距离不满足阈值比例。理论来说 bestDist/bestDist2
                    // 越小越好
                    ...
                    if (static_cast<float>(bestDist1) < mfNNratio * static_cast<float>(bestDist2))
                    {
                        ...
                        if (mbCheckOrientation)
                        {
                            ...
                            if (rot < 0.0)
                                rot += 360.0f;
                            int bin =
                                round(rot * factor);  // 将rot分配到bin组, 四舍五入, 其实就是离散到对应的直方图组中
                            if (bin == HISTO_LENGTH)
                                bin = 0;
 ...
 * @param  vpMatches12 pKF2中与pKF1匹配的MapPoint，vpMatches12[i]表示匹配的地图点，null表示没有匹配，i表示匹配的pKF1
 * 特征点索引
 * @return                    成功匹配的数量
 */
