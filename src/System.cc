  ...
  , mbShutDown(false)
{
    ...
    LOG(INFO) << endl
              << "ORB-SLAM3 Copyright (C) 2017-2020 Carlos Campos, Richard Elvira, Juan J. Gómez, José M.M. Montiel "
                 "and Juan D. Tardós, University of Zaragoza."
              << endl
              << "ORB-SLAM2 Copyright (C) 2014-2016 Raúl Mur-Artal, José M.M. Montiel and Juan D. Tardós, University "
                 "of Zaragoza."
              << endl
    ...
    if (mSensor == MONOCULAR)
        LOG(INFO) << "Monocular" << endl;  // 单目
    else if (mSensor == STEREO)
        LOG(INFO) << "Stereo" << endl;  // 双目
    else if (mSensor == RGBD)
        LOG(INFO) << "RGB-D" << endl;  // RGBD相机
    else if (mSensor == IMU_MONOCULAR)
        LOG(INFO) << "Monocular-Inertial" << endl;  // 单目 + imu
    else if (mSensor == IMU_STEREO)
        LOG(INFO) << "Stereo-Inertial" << endl;  // 双目 + imu
    else if (mSensor == IMU_RGBD)
        LOG(INFO) << "RGB-D-Inertial" << endl;  // RGBD相机 + imu
    ...
    {
        // Load ORB Vocabulary
        LOG(INFO) << "Loading ORB Vocabulary. This could take a while..." << endl;
        ...
        LOG(INFO) << "Initialization of Atlas from scratch " << endl;
        mpAtlas = new Atlas(0);
    }
    else
    {
        // Load ORB Vocabulary
        LOG(INFO) << endl << "Loading ORB Vocabulary. This could take a while..." << endl;
        ...
        LOG(INFO) << "Load File" << endl;

        // Load the file with an earlier session
        // clock_t start = clock();
        LOG(INFO) << "Initialization of Atlas from file: " << mStrLoadAtlasFromFile << endl;
        bool isRead = LoadAtlas(FileType::BINARY_FILE);

        if (!isRead)
        {
            LOG(INFO) << "Error to load the file, please try with other session file or vocabulary file" << endl;
            exit(-1);
        }
        // mpKeyFrameDatabase = new KeyFrameDatabase(*mpVocabulary);
        ...
        // LOG(INFO) << "Binary file read in " << msElapsed << " ms" << endl;
    ...
    {
        LOG(INFO) << "Discard points further than " << mpLocalMapper->mThFarPoints << " m from current camera" << endl;
                                 ...
                                 const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    {
        LOG(ERROR) << "ERROR: you called TrackStereo but input sensor was not set to Stereo nor Stereo-Inertial."
                   << endl;
        exit(-1);
    }
    ...
    // LOG(INFO) << "start GrabImageStereo" << std::endl;
    Sophus::SE3f Tcw = mpTracker->GrabImageStereo(imLeftToFeed, imRightToFeed, timestamp, filename);

    // LOG(INFO) << "out grabber" << std::endl;
                                    ...
                                    string filename)
{
    ...
    LOG(INFO) << "TrackMonocular --- GrabImuData";
    ...
    // 计算相机位姿
    LOG(INFO) << "TrackMonocular --- GrabImageMonocular";
...
void System::Shutdown()
{
    ...
    while (!mpLocalMapper->isFinished() || !mpLoopCloser->isFinished() || mpLoopCloser->isRunningGBA())
    {
        ...
        //     LOG(INFO) << "mpLocalMapper is not finished" << endl;
        // if(!mpLoopCloser->isFinished())
        //     LOG(INFO) << "mpLoopCloser is not finished" << endl;
        // if(mpLoopCloser->isRunningGBA()){
        //     LOG(INFO) << "mpLoopCloser is running GBA" << endl;
        //     LOG(INFO) << "break anyway..." << endl;
        //     break;
        // }
        usleep(5000);
    }
...
void System::SaveTrajectoryTUM(const string& filename)
{
    ...
    if (mSensor == MONOCULAR)
    {
        LOG(ERROR) << "ERROR: SaveTrajectoryTUM cannot be used for monocular." << endl;
        return;
    }
         ...
         lit != lend; lit++, lRit++, lT++, lbL++)
    {
        ...
        f << setprecision(6) << *lT << " " << setprecision(9) << twc(0) << " " << twc(1) << " " << twc(2) << " "
...
{
    LOG(INFO) << endl << "Saving trajectory to " << filename << " ..." << endl;
    /*if(mSensor==MONOCULAR)
    {
        LOG(ERROR) << "ERROR: SaveTrajectoryEuRoC cannot be used for monocular." << endl;
        return;
    }*/
    ...
    int          numMaxKFs = 0;
    Map*         pBiggerMap;
    LOG(INFO) << "There are " << std::to_string(vpMaps.size()) << " maps in the atlas" << std::endl;
    for (Map* pMap : vpMaps)
    {
        LOG(INFO) << "  Map " << std::to_string(pMap->GetId()) << " has "
    ...
    // LOG(INFO) << "size mlpReferences: " << mpTracker->mlpReferences.size() << endl;
    // LOG(INFO) << "size mlRelativeFramePoses: " << mpTracker->mlRelativeFramePoses.size() << endl;
    // LOG(INFO) << "size mpTracker->mlFrameTimes: " << mpTracker->mlFrameTimes.size() << endl;
    // LOG(INFO) << "size mpTracker->mlbLost: " << mpTracker->mlbLost.size() << endl;
         ...
         lit++, lRit++, lT++, lbL++)
    {
        ...
        while (pKF->isBad())
        {
            ...
            Trw = Trw * pKF->mTcp;
            pKF = pKF->GetParent();
            // LOG(INFO) << "--Parent KF: " << pKF->mnId << endl;
        }

        if (!pKF || pKF->GetMap() != pBiggerMap)
        {
            // LOG(INFO) << "--Parent KF is from another map" << endl;
            continue;
        }
    ...
    // LOG(INFO) << "end saving trajectory" << endl;
    f.close();
    LOG(INFO) << endl << "End of saving trajectory to " << filename << " ..." << endl;
}

void System::SaveTrajectoryEuRoC(const string& filename, Map* pMap)
{
    LOG(INFO) << endl << "Saving trajectory of map " << pMap->GetId() << " to " << filename << " ..." << endl;
    /*if(mSensor==MONOCULAR)
    {
        LOG(ERROR) << "ERROR: SaveTrajectoryEuRoC cannot be used for monocular." << endl;
        return;
...
twb.at<float>(1) << " " << twb.at<float>(2) << " " << q[0] << " " << q[1] << " " << q[2] << " " << q[3] << endl;
...
void System::SaveKeyFrameTrajectoryEuRoC(const string& filename)
{
    ...
    if (!pBiggerMap)
    {
        LOG(INFO) << "There is not a map!!" << std::endl;
        return;
    }
    ...
    for (size_t i = 0; i < vpKFs.size(); i++)
    {
        ...
        if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor == IMU_RGBD)
        {
            ...
            Eigen::Quaternionf q   = Twb.unit_quaternion();
            Eigen::Vector3f    twb = Twb.translation();
            f << setprecision(6) << 1e9 * pKF->mTimeStamp << " " << setprecision(9) << twb(0) << " " << twb(1) << " "
              << twb(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
        }
        else
        {
            Sophus::SE3f       Twc = pKF->GetPoseInverse();
            Eigen::Quaternionf q   = Twc.unit_quaternion();
            Eigen::Vector3f    t   = Twc.translation();
            f << setprecision(6) << 1e9 * pKF->mTimeStamp << " " << setprecision(9) << t(0) << " " << t(1) << " "
...
{
    LOG(INFO) << endl << "Saving keyframe trajectory of map " << pMap->GetId() << " to " << filename << " ..." << endl;
...
/*void System::SaveTrajectoryKITTI(const string &filename)
{
    ...
    if(mSensor==MONOCULAR)
    {
        LOG(ERROR) << "ERROR: SaveTrajectoryKITTI cannot be used for monocular." << endl;
        return;
    }
...
void System::SaveTrajectoryKITTI(const string& filename)
{
         ...
         lit != lend; lit++, lRit++, lT++)
    {
        ...
        f << setprecision(9) << Rwc(0, 0) << " " << Rwc(0, 1) << " " << Rwc(0, 2) << " " << twc(0) << " " << Rwc(1, 0)
          << " " << Rwc(1, 1) << " " << Rwc(1, 2) << " " << twc(1) << " " << Rwc(2, 0) << " " << Rwc(2, 1) << " "
          << Rwc(2, 2) << " " << twc(2) << endl;
    }
    f.close();
}
...
void System::SaveDebugData(const int& initIdx)
{
    ...
    SaveTrajectoryEuRoC("init_FrameTrajectoy_" + to_string(mpLocalMapper->mInitSect) + "_" + to_string(initIdx) +
                        ".txt");
...
void System::SaveAtlas(int type)
{
    ...
    if (!mStrSaveAtlasToFile.empty())
    {
        ...
        {
            LOG(INFO) << "Starting to write the save text file " << endl;
            ...
            oa << mpAtlas;
            LOG(INFO) << "End to write the save text file" << endl;
        }
        else if (type == BINARY_FILE)  // File binary
        {
            LOG(INFO) << "Starting to write the save binary file" << endl;
            std::remove(pathSaveFileName.c_str());
            std::ofstream                   ofs(pathSaveFileName, std::ios::binary);
            ...
            oa << strVocabularyChecksum;
            oa << mpAtlas;
            LOG(INFO) << "End to write save binary file" << endl;
        }
    }
}
...
bool System::LoadAtlas(int type)
{
    ...
    if (type == TEXT_FILE)  // File text
    {
        LOG(INFO) << "Starting to read the save text file " << endl;
        std::ifstream ifs(pathLoadFileName, std::ios::binary);
        if (!ifs.good())
        {
            LOG(INFO) << "Load file not found" << endl;
            return false;
        }
        ...
        ia >> strVocChecksum;
        ia >> mpAtlas;
        LOG(INFO) << "End to load the save text file " << endl;
        isRead = true;
    }
    else if (type == BINARY_FILE)  // File binary
    {
        LOG(INFO) << "Starting to read the save binary file" << endl;
        std::ifstream ifs(pathLoadFileName, std::ios::binary);
        if (!ifs.good())
        {
            LOG(INFO) << "Load file not found" << endl;
        ...
        ia >> strVocChecksum;
        ia >> mpAtlas;
        LOG(INFO) << "End to load the save binary file" << endl;
        isRead = true;
    }
    ...
    if (isRead)
    {
        ...
        {
            LOG(INFO) << "The vocabulary load isn't the same which the load session was created " << endl;
            LOG(INFO) << "-Vocabulary name: " << strFileVoc << endl;
...
string System::CalculateCheckSum(string filename, int type)
{
    ...
    if (!f.is_open())
    {
        LOG(INFO) << "[E] Unable to open the in file " << filename << " for Md5 hash." << endl;
        return checksum;
    }
