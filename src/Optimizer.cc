                                ...
                                int& num_MPs, int& num_edges, bool bLarge, bool bRecInit)
{
    ...
    // Step 3.
    // 固定一帧，为vpOptimizableKFs中最早的那一关键帧的上一关键帧，如果没有上一关键帧了就用最早的那一帧，毕竟目前得到的地图虽然有尺度但并不是绝对的位置
                            ...
                            const bool bAllPoints)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        e12->setVertex(0,
                       dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id2)));  // 2相机坐标系下的三维点
        ...
        if (i2 >= 0)
        {
            ...
            nInKF2++;  // 输出信息，表示在kf2中找到MP2的点数
              ...
              //! 如果没找到，使用三维点投影到KF2中，表示并没有特征点与之对应（把这个结果当做obs2是不是会带来一些误差，而且还不通过内参吗？？？，重大bug）
        {
 ...
 * @param NonCorrectedSim3    通过pKFi->GetPose()计算的放NonCorrectedSim3也就是回环前的位姿
 * 这里面的帧只是与mpCurrentKF相关联的
                                       ...
                                       const map<KeyFrame*, set<KeyFrame*>>& LoopConnections, const bool& bFixScale)
{
    ...
    vector<g2o::Sim3, Eigen::aligned_allocator<g2o::Sim3>> vCorrectedSwc(nMaxKFid +
                                                                         1);  // 存放每一帧优化后的sim3，修正mp位姿用
    // 这个变量没有用
    ...
    // Set Loop edges
    // Step
    // 3：添加第1种边：LoopConnections是闭环时因为MapPoints调整而出现的新关键帧连接关系（包括当前帧与闭环匹配帧之间的连接关系）
    int count_loop = 0;
...
/**
 * @brief  LoopClosing::CorrectLoop() 回环矫正时使用，IMU加视觉，全局本质图优化，流程基本与上面 OptimizeEssentialGraph
 * 一模一样，同样有严重BUG 优化目标： 地图中所有MP与关键帧
 * @param pMap                当前的map
 * @param pLoopKF             mpLoopMatchedKF 与 mpCurrentKF 匹配的关键帧
 * @param pCurKF              mpCurrentKF 当前关键帧
 * @param NonCorrectedSim3    通过pKFi->GetPose()计算的放NonCorrectedSim3也就是回环前的位姿
                                           ...
                                           const map<KeyFrame*, set<KeyFrame*>>& LoopConnections)
{
    ...
    // Set Loop edges
    // 3.
    // 添加边：LoopConnections是闭环时因为MapPoints调整而出现的新关键帧连接关系（包括当前帧与闭环匹配帧之间的连接关系）
    Edge4DoF* e_loop;
    for (map<KeyFrame*, set<KeyFrame*>>::const_iterator mit = LoopConnections.begin(), mend = LoopConnections.end();
         mit != mend; mit++)
    {
 ...
 */
void Optimizer::LocalBundleAdjustment(KeyFrame* pMainKF, vector<KeyFrame*> vpAdjustKF, vector<KeyFrame*> vpFixedKF,
                                      bool* pbStopFlag)
{
    ...
    for (MapPoint* pMPi : vpMPs)
    {
        ...
        g2o::VertexSBAPointXYZ* vPoint =
            static_cast<g2o::VertexSBAPointXYZ*>(optimizer.vertex(pMPi->mnId + maxKFid + 1));
 ...
 * @param vpFixedCorrectedKFs    vpLocalCurrentWindowKFs
 * 当前地图中经过矫正的关键帧，也就是Optimizer::LocalBundleAdjustment中优化过的vpAdjustKF
                                       ...
                                       vector<MapPoint*>& vpNonCorrectedMPs)
{
    ...
    vector<g2o::Sim3, Eigen::aligned_allocator<g2o::Sim3>> vCorrectedSwc(
        nMaxKFid + 1);  // 存放每一帧优化后的sim3，调试输出用，没有实际作用
    vector<g2o::VertexSim3Expmap*> vpVertices(nMaxKFid + 1);  // 存放节点，没用，还占地方
                                ...
                                LoopClosing::KeyFrameAndPose& corrPoses)
{
    ...
    // 2.3
    // 类似于上面的做法如果有前一个关键帧放入lFixedKeyFrames，否则从vpOptimizableKFs取出，注意这里防止重复添加的标识又多了一个变量
    // 记录完全固定的帧
    list<KeyFrame*> lFixedKeyFrames;
    if (vpOptimizableKFs.back()->mPrevKF)
    {
    ...
    for (vector<pair<MapPoint*, int>>::iterator lit = pairs.begin(), lend = pairs.end(); lit != lend; lit++, i++)
    {
             ...
             mit != mend; mit++)
        {
            ...
            if (pKFi->mnBALocalForKF != pCurrKF->mnId && pKFi->mnBAFixedForKF != pCurrKF->mnId)  // If optimizable or
                                                                                                 // already included...
            {
